...
-- checking energy production with respect to time (year, months, weekday, hours)

-- Energy production with respect to Year
SELECT
	DATEPART(year, Time) AS Year,
	ROUND(AVG([Energy_delta(Wh)]), 2) AS Avg_energy_produced,
	SUM([Energy_delta(Wh)]) AS Sum_energy_produced
FROM 
	Sheet1$
GROUP BY
	DATEPART(year, Time)
ORDER BY
	Sum_energy_produced DESC;

--Energy production with respect to accumulated Months
SELECT
	month,
	ROUND(AVG([Energy_delta(Wh)]), 2) AS Avg_energy_produced,
	SUM([Energy_delta(Wh)]) AS Sum_energy_produced
...
-- Energy production with respect to acumulated WeekDays
SELECT
...
-- Energy production with respect to Hours
SELECT
	hour,
	ROUND(AVG([Energy_delta(Wh)]), 2) AS Avg_energy_produced,
	SUM([Energy_delta(Wh)]) AS Sum_energy_produced
...
ORDER BY
	Sum_energy_produced DESC;
/* the above queries were ran to determine seasonal trend and time series pattern of energy production
which will help in proper energy planning, distribution, and forecasting*/

-- Energy production with weathertype
-- OPTIMIZE and NORMAL WITHOUT PERCENTAGE
WITH Total_Energy_Produced AS (
	SELECT 
...
SELECT
	weather_type,
	ROUND(AVG([Energy_delta(Wh)]), 2) AS Average_Energy_Produced,
	SUM([Energy_delta(Wh)]) AS Sum_Energy_Produced,
	ROUND((SUM([Energy_delta(Wh)]) / TE.Total_Energy) * 100,2) AS Percentage_of_Total_Energy
	FROM
		Sheet1$
	CROSS JOIN
		Total_Energy_Produced AS TE
	GROUP BY
...
-- Relationship between solar horizontal irradiance and energy produced
SELECT
	month,
	ROUND(AVG(GHI), 2) AS Average_solar_radiation,
	ROUND(SUM(GHI), 2) AS Total_solar_radiation,
	SUM([Energy_delta(Wh)]) AS Total_Energy_Produced
FROM
	Sheet1$
GROUP BY 
	month
...
--The relationship between temperature and energy production:
SELECT 
	temp,
	ROUND(AVG([Energy_delta(Wh)]), 2) AS avg_energy_production
FROM 
	Sheet1$
GROUP BY
	temp
